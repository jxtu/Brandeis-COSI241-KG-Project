...
def initialize_model_directory(args, random_seed=None):
    ...
    if args.model in ["rule"]:
        ...
        if args.baseline == "avg_reward":
            ...
            print(
                "* Policy Gradient Baseline: average reward baseline plus normalization"
            )
        else:
...
def train(lf):
    ...
    if args.adaptation:
        for x in few_train_data:
            ...
            lf.run_train(
                few_train_data[x], few_dev_data, adaptation=True, adaptation_relation=x
            )
    elif args.few_shot:
        if args.checkpoint_path is not None:
        ...
        if args.few_shot:
            ...
            for x in test_data:
                ...
                test_metrics = src.eval.hits_and_ranks(
                    test_data[x], pred_scores, lf.kg.all_objects, verbose=True
                )
...
def run_ablation_studies(args):
    ...
    to_m_rels, to_1_rels, (to_m_ratio, to_1_ratio) = data_utils.get_relations_by_type(
        args.data_dir, relation_index_path
    )
    relation_by_types = (to_m_rels, to_1_rels)
    to_m_ratio *= 100
    ...
    for system in systems:
        ...
        _, _, _, _, mrr_full_kg = src.eval.hits_and_ranks(
            dev_data, pred_scores, lf.kg.all_objects, verbose=True
        )
        if to_1_ratio == 0:
    ...
    print(
        "{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})".format(
            to_m_ratio,
            to_m_mrrs["ours"][""],
            to_m_mrrs["-rs"][""],
    ...
    print(
        "{:.1f}\t{:.1f}\t{:.1f}".format(
            mrrs["ours"]["full_kg"], mrrs["-rs"]["full_kg"], mrrs["-ad"]["full_kg"]
        )
    )
    print("--------------------------")
    print("Performance w.r.t. relation types")
...
def export_error_cases(lf):
    ...
    src.eval.export_error_cases(
        ...
        pred_scores,
        lf.kg.dev_objects,
        os.path.join(lf.model_dir, "error_cases.pkl"),
    )
...
def compute_fact_scores(lf):
    ...
    dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path)
    test_data = data_utils.load_triples(
        test_path, entity_index_path, relation_index_path
    )
    lf.eval()
    lf.load_checkpoint(get_checkpoint_path(args))
        ...
        with torch.set_grad_enabled(
            ...
            if args.search_random_seed:
                ...
                for i in range(num_runs):
                    ...
                    print(
                        "Hits@1 mean: {:.3f}\tstd: {:.6f}".format(
                            np.mean(hits_at_1s_), np.std(hits_at_1s_)
                        )
                    )
                    print(
                        "Hits@10 mean: {:.3f}\tstd: {:.6f}".format(
                            np.mean(hits_at_10s_), np.std(hits_at_10s_)
                        )
                    )
                    print(
                        "MRR mean: {:.3f}\tstd: {:.6f}".format(
                            np.mean(mrrs_), np.std(mrrs_)
                        )
                    )
                    o_f.write(
                        "Hits@1 mean: {:.3f}\tstd: {:.6f}\n".format(
                            np.mean(hits_at_1s_), np.std(hits_at_1s_)
                        )
                    )
                    o_f.write(
                        "Hits@10 mean: {:.3f}\tstd: {:.6f}\n".format(
                            np.mean(hits_at_10s_), np.std(hits_at_10s_)
                        )
                    )
                    o_f.write(
                ...
                best_random_seed, best_mrr = sorted(
                    mrrs_search.items(), key=lambda x: x[1], reverse=True
                )[0]
                print("* Best Random Seed = {}".format(best_random_seed))
                print(
                    "* @1: {:.3f}\t@10: {:.3f}\tMRR: {:.3f}".format(
                ...
                for i, grid_entry in enumerate(list(grid)):
                    ...
                    best_signature, best_mrr = sorted(
                        mrrs.items(), key=lambda x: x[1], reverse=True
                    )[0]
