...
def beam_search(
    pn, e_s, q, e_t, kg, num_steps, beam_size, return_path_components=False
):
    """
    ...
    def top_k_action(log_action_dist, action_space):
        ...
        # [batch_size, 1]
        action_batch_offset = int_var_cuda(torch.arange(batch_size) * last_k).unsqueeze(
            1
        )
        # [batch_size, k] => [batch_size*k]
    ...
    def top_k_answer_unique(log_action_dist, action_space):
        ...
        log_action_prob = ops.pad_and_cat(
    ...
    if return_path_components:
        ...
        for i in range(batch_size):
            ...
            for k, log_action_prob in enumerate(log_action_probs):
                ...
                e_name = (
                    kg.id2entity[int(search_trace[1][0][i * output_beam_size])]
                    if k == 0
                    else ""
                )
